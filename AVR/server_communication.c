...
//							11	Defined as the drone in the server application
...
// Not fully tested
void send_battery(int16_t level){
	if(!connected) return;
	message_t msg;
	msg.type = TYPE_BATTERY;
	msg.message.battery.level = level;
	uint8_t data[sizeof(battery_message_t)+1];
	memcpy(data, (uint8_t*) &msg, sizeof(data));
	if(use_arq[TYPE_BATTERY]) arq_send(server_connection, data, sizeof(data));
	else simple_p_send(SERVER_ADDRESS, data, sizeof(data));
}
