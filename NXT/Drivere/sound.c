...
const unsigned long sample_pattern[33] =
  {
...
volatile unsigned char sound_mode = SOUND_MODE_NONE;

__irq __arm void sound_isr_C(void);
...
struct {
  ...
  volatile unsigned char buf_id;
  // Double buffer
  volatile unsigned long buf[2][PDM_BUFFER_LENGTH];
  // Amplification LUT
  volatile unsigned char amp[257];
  // Volume val used to create the amp LUT
  volatile signed long cur_vol;
  // Clock divisor to use to play the sample
  volatile unsigned long clock_div;
  // Size of the sample
  volatile unsigned long len;
  // Callback of the stream end
  unsigned char (*callback)(void);
  // Clock divisor to use to play the stream
  volatile unsigned long stream_clock_div;
} sample;
 ...
 */
//const unsigned char sine[] =  {0xa8, 0xc0, 0xca, 0xd4, 0xe0, 0xe9, 0xff, 0xff, 0xff, 0xff, 0xe9, 0xe0, 0xd4, 0xca, 0xc0, 0xa8};
//const unsigned char sine[] =  {0xb0, 0xc0, 0xca, 0xd4, 0xe0, 0xe9, 0xf2, 0xff, 0xff, 0xf2, 0xe9, 0xe0, 0xd4, 0xca, 0xc0, 0xb0};
//const unsigned char sine[] =  {0xc0, 0xc8, 0xd0, 0xd8, 0xe0, 0xea, 0xf4, 0xff, 0xff, 0xf4, 0xea, 0xe0, 0xd8, 0xd0, 0xc8, 0xc0};
const unsigned char sine[] =  {0xc0, 0xc8, 0xd0, 0xd8, 0xe0, 0xea, 0xf4, 0xff, 0xff, 0xf0, 0xe5, 0xdc, 0xd4, 0xcc, 0xc4, 0xbc};
//const unsigned char sine[] =  {0x98, 0xb0, 0xc7, 0xda, 0xea, 0xf6, 0xfd, 0xff, 0xff, 0xfd, 0xf6, 0xea, 0xda, 0xc7, 0xb0, 0x98};
...
const unsigned long silence[16] = {
...
//const unsigned char logvol[] = {0, 4, 16, 28, 40, 52, 64, 78, 93, 109, 128, 196, 255, 255};
//const unsigned char logvol[] = {0, 8, 24, 40, 56, 80, 104, 128, 160, 208, 255, 255};
const unsigned char logvol[] = {0, 8, 24, 40, 56, 80, 104, 128, 162, 196, 255, 255};
...
void sound_init()
{
  ...
  *AT91C_SSC_CR = AT91C_SSC_TXEN;       
  ...
  AT91F_AIC_ConfigureIt( AT91C_BASE_AIC, AT91C_ID_SSC, AT91C_AIC_PRIOR_LOWEST, AT91C_AIC_SRCTYPE_INT_EDGE_TRIGGERED, ( void (*)(void) ) sound_isr_C );
...
static void create_tone(const unsigned char *lookup, int lulen, unsigned long *pat, int len)
{
  ...
  unsigned long bit2 = 0x00000001;
  int i = numsamples/step;;
  int error = 0;
  int error2 = 0;
  int out=0;
  unsigned long bits = 0;
  unsigned long bits2 = 0;
  int entry = 0;
...
void sound_freq_vol(unsigned long freq, unsigned long ms, int vol)
{
  ...
  sample.len = len;
  sample.ptr = (unsigned char *)sample.buf[buf];
  sample.buf_id = buf;
...
void sound_fill_sample_buffer(void) {
  sample.buf_id ^= 1;
  volatile unsigned long *sbuf = sample.buf[sample.buf_id];
  unsigned char i;
  /* Each 8-bit sample is turned into 8 32-bit numbers, i.e. 256 bits altogether */
  for (i = 0; i < PDM_BUFFER_LENGTH >> 3; i++) {
    unsigned char smp = (sample.count > 0 ? sample.amp[*sample.ptr] : 128);
    unsigned char msk = "\x00\x10\x22\x4a\x55\x6d\x77\x7f"[smp & 7];
    unsigned char s3 = smp >> 3;
...
/*
      ...
      unsigned long msb = 0xffffffff << (32 - (smp >> 3));
...
{
  if (data == (unsigned char *) 0 || length == 0) return;
  ...
  unsigned long cdiv = (OSC/(2*SAMPBITS) + freq/2)/freq;
  set_vol(vol);
...
void sound_play_stream_setup(unsigned long freq, int vol)
{
  ...
  sample.ptr = (unsigned char *)silence;
...
__irq __arm void sound_isr_C(void)
{
    ...
    else
    {
      ...
      sample.count = SILENCE_CNT;
      sample.len = 16;
      AT91F_AIC_Trig(AT91C_BASE_AIC, AT91C_ID_SSC);
     // sound_isr_C();
    }
