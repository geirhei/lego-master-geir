...
typedef struct{
  ...
  signed char output_percent[NXT_AVR_N_OUTPUTS];
...
static unsigned char data_to_avr[5 + NXT_AVR_N_OUTPUTS];


typedef struct {
  // Raw values
  unsigned short adc_value[NXT_AVR_N_INPUTS];
  unsigned short buttonsVal;
...
static unsigned short prev_buttons;
static unsigned short button_state;
static unsigned short debounce_state;
static unsigned short debounce_cnt;
...
static void nxt_avr_start_read(void)
{
  ...
  twi_start_read(NXT_AVR_ADDRESS, (unsigned char *)(&data_from_avr[from_buf]), sizeof(from_avr));
}
...
{
  unsigned long checkByte = 0;
  unsigned char *a = data_to_avr;
  unsigned char *b = (unsigned char *) (&io_to_avr);
  unsigned char *e = b + sizeof(io_to_avr);
...
void nxt_avr_link_init(void)
{
  twi_start_write(NXT_AVR_ADDRESS, (const unsigned char *) avr_brainwash_string,
...
static struct {
  ...
  unsigned long still_busy;
  unsigned long not_ok;
} nxt_avr_stats;
...
static void nxt_avr_unpack(void)
{
  ...
  // calc the checksum
  p = (unsigned char *) (&data_from_avr[from_buf]);
...
static unsigned long update_count;
static unsigned long link_state = LS_CLOSED;
...
unsigned long battery_voltage(void)
{
  ...
  unsigned long voltageVal = io_from_avr->extra;
...
void nxt_avr_set_input_power(unsigned long n, unsigned long power_type)
{
  ...
  if (n < NXT_AVR_N_INPUTS && power_type <= 2) {
    unsigned char val = (power_type & 0x2 ? 0x10 << n : 0) | ((power_type & 1) << n);
