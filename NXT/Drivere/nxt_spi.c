...
const unsigned char *display = (unsigned char *) 0;
volatile unsigned char dirty = 0;
volatile unsigned char page = 0;
volatile const unsigned char *data = (unsigned char *) 0;
unsigned char mode = 0xff;

__irq __arm void spi_isr_C(void);
extern void spi_isr_entry(void);
  
static void spi_set_mode(unsigned char m)
{
...
__irq __arm void spi_isr_C(void)
{
   ...
   */
  *AT91C_SPI_TNPR = (unsigned long) data;
  *AT91C_SPI_TNCR = 132;
  page = (page + 1) % 8;
  data += 100;
...
#define SPI_BITRATE 2000000
  ...
  *AT91C_SPI_MR = AT91C_SPI_MSTR|AT91C_SPI_MODFDIS | (0xB<<16);
  ...
  data = display = (unsigned char *) 0;
  dirty = 0;
  page = 0;

  /* Install the interrupt handler */  extern void spi_isr_entry(void);

   AT91F_AIC_ConfigureIt( AT91C_BASE_AIC, AT91C_ID_SPI, AT91C_AIC_PRIOR_HIGHEST-2, AT91C_AIC_SRCTYPE_INT_LEVEL_SENSITIVE, ( void (*)(void) ) spi_isr_C);
...
{
  unsigned long status;
  unsigned long cd_mask = (CD ? 0x100 : 0);
