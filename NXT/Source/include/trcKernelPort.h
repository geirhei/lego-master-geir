...
#define TRC_FREERTOS_VERSION_9_0_2				7 /* If using FreeRTOS v9.0.2 or later version */
...
#ifndef TRC_CFG_INCLUDE_NOTIFY_EVENTS
...
#define TRACE_GET_TASK_FILTER(pxTask) prvTraceGetTaskNumberHigh16(pxTask)
#define TRACE_SET_TASK_FILTER(pxTask, group) prvTraceSetTaskNumberHigh16(pxTask, group)

#define TRACE_GET_OBJECT_FILTER(pxObject) prvTraceGetObjectNumberHigh16(pxObject)
#define TRACE_SET_OBJECT_FILTER(pxObject, group) prvTraceSetObjectNumberHigh16(pxObject, group)
...
#define TRACE_GET_OBJECT_NUMBER(CLASS, pxObject) (traceHandle)(prvTraceGetObjectNumberLow16(pxObject))
...
#define traceQUEUE_RECEIVE( pxQueue ) \
	if (isQueueReceiveHookActuallyPeek){ trcKERNEL_HOOKS_KERNEL_SERVICE(PEEK, SUCCESS, TRC_UNUSED, pxQueue) } else { trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE, SUCCESS, TRC_UNUSED, pxQueue); }  \
...
#define traceQUEUE_RECEIVE_FAILED( pxQueue ) \
	if (isQueueReceiveHookActuallyPeek){ trcKERNEL_HOOKS_KERNEL_SERVICE(PEEK, FAILED, TRC_UNUSED, pxQueue); } else { trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE, FAILED, TRC_UNUSED, pxQueue); }

/* Called when the task is blocked due to a receive operation on an empty queue */
#undef traceBLOCKING_ON_QUEUE_RECEIVE
#define traceBLOCKING_ON_QUEUE_RECEIVE( pxQueue ) \
	if (isQueueReceiveHookActuallyPeek){ trcKERNEL_HOOKS_KERNEL_SERVICE(PEEK, BLOCK, TRC_UNUSED, pxQueue); } else { trcKERNEL_HOOKS_KERNEL_SERVICE(RECEIVE, BLOCK, TRC_UNUSED, pxQueue); } \
...
#define traceQUEUE_PEEK_FAILED( pxQueue ) \
	trcKERNEL_HOOKS_KERNEL_SERVICE(PEEK, FAILED, TRC_UNUSED, pxQueue);

/* Called on xQueuePeek blocking (added in FreeRTOS v9.0.2) */
#undef traceBLOCKING_ON_QUEUE_PEEK
#define traceBLOCKING_ON_QUEUE_PEEK( pxQueue ) \
	trcKERNEL_HOOKS_KERNEL_SERVICE(PEEK, BLOCK, TRC_UNUSED, pxQueue); \
	if (TRACE_GET_OBJECT_TRACE_CLASS(TRC_UNUSED, pxQueue) != TRACE_CLASS_MUTEX) \
	{trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();}
...
#define traceTIMER_EXPIRED(tmr) \
	prvTraceStoreKernelCall(TIMER_EXPIRED, TRACE_CLASS_TIMER, TRACE_GET_TIMER_NUMBER(tmr));
...
#if (TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_NOTIFY_EVENTS == 1)
...
#endif /* if (TRC_CFG_INCLUDE_NOTIFY_EVENTS == 1) */
...
#undef traceQUEUE_CREATE
#define traceQUEUE_CREATE( pxNewQueue )\
	TRACE_SET_OBJECT_FILTER(pxNewQueue, CurrentFilterGroup); \
...
#undef traceCREATE_COUNTING_SEMAPHORE
	...
	#define traceCREATE_COUNTING_SEMAPHORE() \
		prvTraceStoreEvent2(PSF_EVENT_SEMAPHORE_COUNTING_CREATE, (uint32_t)xHandle, (( xQUEUE * ) xHandle)->uxMessagesWaiting);
#else
	#define traceCREATE_COUNTING_SEMAPHORE() \
...
#define traceQUEUE_SEND( pxQueue ) \
	if (TRACE_GET_TASK_FILTER(TRACE_GET_CURRENT_TASK()) & CurrentFilterMask) \
	if (TRACE_GET_OBJECT_FILTER(pxQueue) & CurrentFilterMask) \
	switch (pxQueue->ucQueueType) \
	{ \
		case queueQUEUE_TYPE_BASE: \
...
#define traceGIVE_MUTEX_RECURSIVE( pxMutex ) \
	if (TRACE_GET_TASK_FILTER(TRACE_GET_CURRENT_TASK()) & CurrentFilterMask) \
	if (TRACE_GET_OBJECT_FILTER(pxMutex) & CurrentFilterMask) \
...
#endif /* (TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_NOTIFY_EVENTS == 1) */
